...
public class UpdateHOD 
{
    public boolean isUpdate(DTO dt) 
    {
        ...
        try
        {
            ...
            pst = con.prepareStatement("update hod set Name=?,Faculty_id=?,Address=?,Joining_Year=?,Adhar_Number=?,Blood_Group=?,Branch=?,Gender=?,Mobile_Number=?,Date_of_Birth=?,email=? where Faculty_id = '"+enrollment+"'");
